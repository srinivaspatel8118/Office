...
public class WorkingOnExcel {
	...
	public WorkingOnExcel(String path) throws IOException{
		this.path=path;
	...
	public ArrayList<Login2> getExcelData(String sheetName,String colName,String methodName){
		int rowCount = getRowCount(sheetName,colName,methodName);
		int colCount =sheet.getRow(0).getLastCellNum();
		String data[][] = new String[rowCount][colCount];
		ArrayList<Login2> list=new ArrayList<>();
		int firstRow=getFirstDataRowNum(sheetName,colName,methodName);
		for (int i = 0; i < rowCount; i++) {
			for (int j = 0; j < colCount; j++) {
				if(!sheet.getRow(firstRow+i).getCell(j).getStringCellValue().equals(""))
			...
			list.add(new Login2(data[i][0],data[i][1],data[i][2],data[i][3]));	
		}
		return list;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		WorkingOnExcel excel=new WorkingOnExcel(System.getProperty("user.dir")+"//src//dataproviders//Vendors.xlsx");
		
		System.out.println(excel.getRowNo("sheet1", "tc_CreateVendor1"));
		System.out.println(excel.getRowCount("sheet1"));
		System.out.println(excel.getRowCount("sheet1", "TCID", "tc_CreateVendor"));
		System.out.println(excel.getCellData("sheet1", "TCID", 4));
	}
